...
export const __inner = function() {}

__inner.mergeOrigin = (a, lo, hi, mid) => {
  if(lo === hi) return
  let i = lo,
      j = mid + 1,
      aux = []
...
__inner.mergeNoEdgeCheck = (a, lo, hi, mid) => {
  let aux = []
...
__inner.mergeDraw = function*(a, lo, hi, mid, ctx) {
...
__inner.sortBUxxx = (a, lo, hi, merge) => {
  const N = a.length
  let mid = 0
...
__inner.sortBURecursive = (a, lo, hi, merge) => {
  const N = a.length
  const iter = (a, sz, lo) => {
    if(sz >= N) return
...
__inner.drawSorting = function*(a, lo, hi, merge, ctx, w, h) {
  if(lo === hi) return
  let mid = Math.floor((lo + hi) / 2)
